...
<html>
				...
				<s:iterator id="order" value="#pageVO.getData()">
					...
					<div class="line_H overflowH">
						...
						<div class="list_width2 textstyle02">
							<s:if test="#order.statusVO==@com.banche.enums.OrderStatus@ACCEPTED_BUT_NOTICE.getName()">
								<a class="sendNotice" type="${order.id }" href="javascript:void(0);">${order.statusVO }</a>
							</s:if>
							<s:else>
								${order.statusVO }
							</s:else>
						</div>
						<div class="list_width02 view_detail view_h" auth="bc_Order_viewAccepting">
	...
	<script type="text/javascript">
		$('.sendNotice').click(function(){
			...
			$.DJconfirm("要发送已接单通知吗？", function(){
				$.getJSON("bc_Order_noticeAccepted?orderId="+id, function(data){
					if(data.status==1){					
						alert('发送通知成功');						
					}else{
						alert(data.info);
