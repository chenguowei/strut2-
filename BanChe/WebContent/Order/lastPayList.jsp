...
<html>
			...
			<div class="table_2">
				...
				<s:iterator id="order" value="#pageVO.getData()">
					...
					<div class="line_H overflowH">
						...
						<s:if test="#order.statusVO==@com.banche.enums.OrderStatus@LAST_PAY.getName()">
							<div class="list_width2 textstyle02"><a class="sendNotice" type="${order.id }" href="javascript:void(0);">${order.statusVO }</a></div>
						</s:if>
						<s:else>
							<div class="list_width2 textstyle01">${order.statusVO }</div>
						</s:else>						
						<div class="list_width02 view_detail view_h" auth="bc_Order_viewLastPay">
	...
	<script type="text/javascript">
		$('.sendNotice').click(function(){
			...
			$.DJconfirm("要发送已收尾款通知吗？", function(){
				$.getJSON("bc_Order_noticeLastPay?orderId="+id, function(data){
					if(data.status==1){
						alert("发送通知成功");						
					}else{
						alert(data.info);
